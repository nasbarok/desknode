...
NON_JOUABLES=(
  ...
  "verif_harnais_dn81.py|CAUSE A — le cockpit de planification est un depot PRIVE, ⛔ jamais clone a cote du code. Les 87 ancres 📍 que (c2) rejoue vivent dans son ledger : sans lui, (c2) n'a AUCUNE population. ⚠️ (c1) et (c3), eux, sont JOUABLES dans un clone nu — mais la gate ⛔ ne se coupe PAS en deux : un verdict partiel publie sous la meme ligne de BILAN serait indiscernable d'un verdict complet, et c'est le defaut que dn8-1 existe pour fermer. ⇒ elle rend 4 EN AMONT, et le dit. ⛔ CE N'EST PAS UN SKIP : la regle (3) la joue QUAND MEME pour verifier qu'elle rend bien ce 4.|${TEMOIN_COCKPIT}|AUCUN|4"
