...
| **3** | `CONTRIBUTING.md` : *« Your **DeskNode version** — it is **displayed on the device**. »* | 🔴 **AUCUNE version n'est affichée sur la dalle.** **0** appel à `esp_app_get_description` sur les **54** fichiers de `firmware/desknode/main/` (`main` : 0 sur 48). `dn_ui.c` l'écrit pour le MENU : *« pas de reboot, **pas de version** »*. Le seul identifiant est `App version: <sha>`, **au bandeau SÉRIE de boot** | ✅ **SOLDÉE ICI** — décision owner du 2026-09-02 : `CONTRIBUTING.md` **demande ce qui existe** (le SHA du bandeau série) **et dit comment l'obtenir**. ⚠️ Une **vraie** version affichée + le tag `v0.1.0-beta` restent à **`dn8`** <br><br>⚠️ **ANNOTÉ LE 2026-09-13 par `dn8-6` — la mesure d'origine ⛔ n'est pas effacée : elle était vraie le 2026-09-02, et elle l'est restée jusqu'à `dn8-6`.** Le firmware de `dn8-6` affiche la version **dans l'en-tête du MENU, sous le titre** (`version <sha>`), **lue dans le descripteur du binaire** (`esp_app_get_description()->version`, ⛔ pas une constante) ; un build sans git y montre `version ?`. `CONTRIBUTING.md` et le formulaire de bug le disent désormais. ⚠️ **Un firmware plus ancien n'affiche rien** : il se lit toujours au **bandeau série** `App version`. ⚠️ Au jour de cette annotation, la ligne est **construite et compilée, ⛔ pas encore vue sur la dalle** : l'œil de l'owner appartient à la séance carte de `dn8-6`. Le tag `v0.1.0-beta` reste à **`dn8`** |
