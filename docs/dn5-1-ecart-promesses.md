...
| **le poids d'un clone** | 🔴 **NON MESURÉ** ici (bundle `main` ≈ 31 Mo au cadrage) **[ANNOTÉ LE 2026-09-05 (`dn5-5`) : ✅ **MESURÉ** désormais, par un `git clone` RÉEL dans un dossier neuf hors de l'arborescence, ⛔ pas par `du`. **Clone complet : 31,94 Mio reçus** (3 875 objets, 451 commits, les 2 branches publiées) · **`--depth 1` : 30,22 Mio** (637 objets). Les deux sont publiés dans `CONTRIBUTING.md` § *What cloning this costs*, chacun **avec à qui il sert** et **avec la commande qui le reproduit**. L'écart avec `du -sb .git` (**134 337 533 o**) y est expliqué **à l'octet** — et ⚠️ il n'a **pas une seule cause** : les objets libres n'en portent que **46,7 %**, la moitié la plus grosse étant que le pack local couvre **4 branches locales** qu'un clone ne demande jamais. ⚠️ L'ordre de grandeur « ≈ 31 Mo » du cadrage était **juste** ; ce qu'il ne disait pas, c'est **à qui chaque chiffre sert** et que **`--depth 1` ne fait gagner que 1,72 Mio (~5 %)**, parce que le poids est dans les fichiers de la version courante (46,1 Mo), ⛔ pas dans l'historique. Captures : `mesures/dn5-5/T1-clone-complet.txt`, `T2-clone-depth1.txt`, `T3-ecart-du-vs-clone.txt`. ⛔ La ligne se DATE, elle ne se réécrit pas]** | **`dn5-5`** — ✅ soldé le 2026-09-05 |
