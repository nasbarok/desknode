...
| **2** | `CONTRIBUTING.md` : *« A CLA is required… **A bot handles it in one click**. »* | 🔴 **0 fichier sous `.github/`**, ni sur `main` ni sur `HEAD`. Il n'y a **aucun bot**. | ✅ **SOLDÉE le 2026-09-02 par `dn5-2` / AC2.4** — ⛔ la ligne d'origine n'est pas effacée : elle disait vrai le 2026-09-02 au matin. L'artefact existe désormais : `CLA.md` (le document, qui n'existait pas non plus) et `.github/workflows/cla.yml` (l'action, **épinglée** `contributor-assistant/github-action@v2.6.1` — dépôt **archivé** depuis le 2026-03-23, et c'est écrit dans `CONTRIBUTING.md`). 🎯 **VU TOURNER, ⛔ pas lu dans une doc** : PR interne **#1** du 2026-09-02, **3 runs**, le bot a **commenté** (12:43:01Z), le magasin `.github/cla-signatures.json` a été **créé par le bot** (commit `bfd63ae` sur `main`), coût **2,0 min Actions / 0,00 $**. ⚠️ Le run est en `failure` — **c'est le bot qui fait son travail** : *« Committers of pull request 1 have to sign the CLA »*. Relevé complet : `mesures/dn5-2/T3-essai-cla.txt`. ⚠️ **Ce qui reste, et qui ne peut pas se faire ici** : la preuve d'exécution sur une PR d'un contributeur **externe** — un dépôt à **0 fork** et **1 collaborateur** n'en reçoit aucune. Porteur **`dn8`**, écrit dans `CONTRIBUTING.md`, `CHANGELOG.md` et `docs/roadmap.md` <br><br>⚠️ **ANNOTÉ LE 2026-09-15 par `dn8-8` — ⛔ rien n'est effacé.** *« Un dépôt à **0 fork** et **1 collaborateur** n'en reçoit aucune »* a cessé de décrire le dépôt ce jour : il est **public**, et il peut être forké. La preuve, elle, ⛔ n'existe toujours pas — elle attend la **première PR d'un contributeur externe réel**, et personne n'en ouvre une exprès pour la produire |
| **3** | `CONTRIBUTING.md` : *« Your **DeskNode version** — it is **displayed on the device**. »* | 🔴 **AUCUNE version n'est affichée sur la dalle.** **0** appel à `esp_app_get_description` sur les **54** fichiers de `firmware/desknode/main/` (`main` : 0 sur 48). `dn_ui.c` l'écrit pour le MENU : *« pas de reboot, **pas de version** »*. Le seul identifiant est `App version: <sha>`, **au bandeau SÉRIE de boot** | ✅ **SOLDÉE ICI** — décision owner du 2026-09-02 : `CONTRIBUTING.md` **demande ce qui existe** (le SHA du bandeau série) **et dit comment l'obtenir**. ⚠️ Une **vraie** version affichée + le tag `v0.1.0-beta` restent à **`dn8`** <br><br>⚠️ **ANNOTÉ LE 2026-09-13 par `dn8-6` — la mesure d'origine ⛔ n'est pas effacée : elle était vraie le 2026-09-02, et elle l'est restée jusqu'à `dn8-6`.** Le firmware de `dn8-6` affiche la version **dans l'en-tête du MENU, sous le titre** (`version <sha>`), **lue dans le descripteur du binaire** (`esp_app_get_description()->version`, ⛔ pas une constante) ; un build sans git y montre `version ?`. `CONTRIBUTING.md` et le formulaire de bug le disent désormais. ⚠️ **Un firmware plus ancien n'affiche rien** : il se lit toujours au **bandeau série** `App version`. ⚠️ Au jour de cette annotation, la ligne est **construite et compilée, ⛔ pas encore vue sur la dalle** : l'œil de l'owner appartient à la séance carte de `dn8-6`. Le tag `v0.1.0-beta` reste à **`dn8`** <br><br>⚠️ **ANNOTÉ LE 2026-09-14 par `dn8-7` — ⛔ rien n'est effacé.** Deux phrases de cette case ont cessé d'être vraies : la ligne de version a été **vue sur la dalle** le 2026-09-13 (séance carte de `dn8-6`, verbatim owner *« version 55006c1 lisible ok (mais a la limite) »*), et **le tag `v0.1.0-beta` existe** — tag annoté et pre-release GitHub, posés par `dn8-7`. Ce qui reste à `dn8` est la **bascule publique** : tant que le dépôt est privé, ni le tag ni la release ne sont atteignables par un inconnu <br><br>⚠️ **ANNOTÉ LE 2026-09-15 par `dn8-8` — ⛔ rien n'est effacé.** La bascule publique a été jouée ce jour : le dépôt est public, et le tag comme la release sont désormais ceux d'un dépôt public ; ce qu'un inconnu en lit sans compte s'écrit ailleurs, une fois mesuré. Il ne reste rien à `dn8` sur cette ligne |
...
| **5** | Le dépôt publie `tools/run_gates.sh` et **les gates que son glob découvre**, sous une convention qui promet qu'elles passent <br><br>⚠️ **ANNOTÉ LE 2026-09-02 par `dn4-40` — LE COMPTE ÉCRIT A ÉTÉ RETIRÉ DE L'ÉNONCÉ, ⛔ pas corrigé.** La ligne d'origine disait **26 gates**. Elle est devenue fausse deux fois en trois jours : le glob en trouvait **27** au cadrage de `dn4-40`, puis **28** dès qu'elle en a ajouté une. 🎯 **Un compte écrit se périme le jour où on ajoute une gate — c'est-à-dire le jour où il compte.** ⇒ l'énoncé nomme désormais LE GLOB, et le chiffre se lit sur la ligne de `BILAN` du runner. ⚠️ Les deux autres `26` de cette page (§ *Les mesures*) sont des **relevés DATÉS** : ⛔ ils ne se réécrivent pas (NFR3). | 🔴 **1 gate ROUGE au moment du push** : `tools/verif_dossier_dn415.py`, **17 OK / 10 KO**. ⚠️ **Elle PRÉ-EXISTE**, ⛔ elle n'est pas de cette story : ses 10 KO portent **tous** sur des fichiers du **cockpit** (`_bmad-output/…`, préfixe `cockpit:` dans sa sortie), qui **ne sont pas dans le clone** | ⛔ **`dn4-39` / `dn4-40`** — ⛔ **ni imputée, ni aggravée, ni réparée ici** ; **déclarée** parce que taire une gate rouge qu'on publie serait le défaut de classe <br><br>⚠️ **ANNOTÉ LE 2026-09-02 par `dn4-39` — la disposition CHANGE DE MAIN, la ligne d'origine ⛔ ne se réécrit pas.** La CI existe désormais (`.github/workflows/gates.yml`), et elle **ne verra JAMAIS ces 10 KO** : ils portent sur le cockpit, qui n'est pas dans le clone. 🔬 **MESURÉ** — là où le cockpit est présent la gate rend **17 OK / 10 KO** ; là où il ne l'est pas elle rendait **1 OK / 1 KO** et s'arrêtait. **Ce n'est donc pas le même rouge des deux côtés.** ⇒ hors du poste, l'absence du cockpit est désormais un **PRÉREQUIS ABSENT** (`rc=4`, avec son motif), **déclaré** dans la table `NON_JOUABLES` de `tools/run_gates.sh` — ⛔ pas un skip, et la gate **dit elle-même** qu'elle ne prétend pas garder ce qu'elle ne peut pas voir. ⛔ **Le remède d'ancrage par motif reste `dn4-40`**, inchangé. |
| **6** | Le dépôt se veut clonable — `CONTRIBUTING.md` invite à contribuer, `run_gates.sh` à vérifier | 🔴 **4 outils de `tools/` ont une dépendance FONCTIONNELLE à un chemin personnel** (⚠️ **le chiffre `3` ci-dessous est CORRIGÉ le 2026-09-02, voir §5**) : `verif_dossier_d5_dn45.py:47-48` (`/home/nasbarok/projects/{desknode,compagnon_project}` en **absolu**), `verif_dossier_dn415.py:168` et `verif_ledger_dn416.py:148` (`~/projects/compagnon_project`). ⇒ sur un clone neuf **elles ne peuvent pas s'exécuter**. S'y ajoutent **5** fichiers `firmware/…/fonts/dn_font_*.c` qui portent `/home/nasbarok/…` dans un commentaire de générateur | ⛔ **`dn5-3`** — ⚠️ **ANNOTATION** : le périmètre de `dn5-1` et l'epic écrivent *« les **2** dépendances fonctionnelles de `tools/` »*. **Mesuré le 2026-09-02 : elles sont TROIS.** ⛔ La ligne ne se réécrit pas, elle se **date** (NFR3) <br><br>✅ **SOLDÉE LE 2026-09-04 PAR `dn5-3` — ⛔ la disposition change, la ligne d'origine ⛔ ne se réécrit pas.** 🔴 **ET LE COMPTE A ENCORE BOUGÉ, DANS L'AUTRE SENS : il vaut 3, ⛔ pas 4.** `dn4-40` a refermé `verif_dossier_dn415.py` et `verif_ledger_dn416.py` en passant (commit `f4848e4`) <br><br>🔴 **CORRIGÉ À LA REVUE DE CODE DU 2026-09-04 — L'ATTRIBUTION CI-DESSUS EST FAUSSE, ET ⛔ ELLE N'EST PAS EFFACÉE.** Ce n'est **pas** `dn4-40` / `f4848e4` qui a refermé `verif_dossier_dn415.py` et `verif_ledger_dn416.py`. Rejoué : à **`f4848e4^`** les deux fichiers portent **déjà** les trois propriétés (`dirname(abspath(__file__))`, `add_argument("--cockpit"`, défaut dérivé de `HOME`) — `f4848e4` n'en a introduit **aucune**. La racine `__file__` et `--cockpit` datent du **2026-08-30** (`eb61b58` = `dn4-15`, `7f7f946` = `dn4-16`) ; le littéral du chemin personnel disparaît à **`f5e9d69`, le 2026-09-02 — `dn4-39`**. ⇒ **le porteur du solde est `dn4-39`**, et le récit *« 2 → 3 → 4 → 3 »* garde ses chiffres mais change de main. — elles dérivent leur racine de `__file__` et prennent `--cockpit`. ⇒ le chemin du chiffre est **2 → 3 → 4 → 3**, et chaque étape est datée plutôt que corrigée. 🆕 **Une troisième dépendance que personne n'avait nommée** a été trouvée par un **5ᵉ motif**, le compte Windows de l'auteur : `tools/thermique_ventilos_dn48.py:310`, dont le défaut `--csv` pointait `C:\Users\<auteur>\AppData\Local\Temp\` et que `enregistrer()` **ouvre en écriture** — un échec ~~**TARDIF**~~ **IMMÉDIAT ET MUET** *(corrigé le 2026-09-04 : `enregistrer()` écrit `<csv>.pid` **avant** son premier `print` ⇒ rc 1, stdout **vide** — ⛔ le mot « tardif » était faux, et il n'est pas effacé)*. 🔬 **Les trois sont soldées** : racine dérivée de `__file__` pour la gate et pour `bench_lisseur_dn45.py`, `tempfile.gettempdir()` pour le défaut `--csv`. ⚠️ **Les 5 polices, elles, RESTENT INTACTES** — leur ligne `Opts:` dit la commande qui a réellement produit le fichier, et `CONTRIBUTING.md` dit désormais pourquoi. ⛔ **AUCUNE entrée n'est ajoutée à cette page** : le compte reste **6**, et la phrase *« six entries »* de `CONTRIBUTING.md` reste vraie |
...
| **DeskNode version** | ✅ **OUI** — `CONTRIBUTING.md` demande le SHA du bandeau série **et dit comment le lire**. ⚠️ Une version *affichée sur la dalle* n'existe toujours pas **[ANNOTÉ LE 2026-09-13 (`dn8-6`) : elle existe désormais — la ligne sous le titre du MENU, lue dans le descripteur du binaire ; firmware plus ancien : le bandeau série `App version`. ⚠️ Construite et compilée, ⛔ pas encore vue sur la dalle (séance carte de `dn8-6`). ⛔ La ligne se DATE, elle ne se réécrit pas]** | `dn8` (version affichée + tag `v0.1.0-beta`) — version affichée : **`dn8-6`** ; tag : `dn8` **[ANNOTÉ LE 2026-09-14 (`dn8-7`) : la version affichée a été vue sur la dalle le 2026-09-13, et le tag `v0.1.0-beta` existe, avec sa pre-release. Reste à `dn8` : la bascule publique. ⛔ La ligne se DATE, elle ne se réécrit pas]** |
...
| **le bot CLA** | 🔴 **NON** — il n'existe pas **[ANNOTÉ LE 2026-09-02 (`dn5-2`) : ✅ **OUI** désormais — `CLA.md` + `.github/workflows/cla.yml`. ⚠️ Ce qui reste est la preuve sur une PR **externe**, impossible à 0 fork / 1 collaborateur]** **[ANNOTÉ LE 2026-09-15 (`dn8-8`) : le dépôt est **public** depuis ce jour et peut être forké — « 0 fork / 1 collaborateur » ne l'empêche plus. La preuve attend la première PR d'un contributeur externe réel. ⛔ La ligne se DATE, elle ne se réécrit pas]** | **`dn5-2` / AC2.4** — ✅ posé · preuve externe ⇒ **`dn8`** |
| **sortie console série** | ✅ **OUI** — `README.md` donne le baud (**115200**), `tools/dn_console.py`, `idf.py monitor` et `miniterm` | — |
| **les gates tournent sur un clone** | 🔴 **NON** — 3 outils exigent un chemin personnel absent du clone **[ANNOTÉ LE 2026-09-04 (`dn5-3`) : ✅ **OUI** désormais. ⛔ Le chiffre `3` ne se réécrit pas — il était juste le 2026-09-02, et il l'est redevenu le 2026-09-04 **pour d'autres outils** : `dn4-40` en a refermé deux, un cinquième motif en a révélé un que personne n'avait nommé. Les trois d'aujourd'hui sont `verif_dossier_d5_dn45.py`, `bench_lisseur_dn45.py` et `thermique_ventilos_dn48.py`, et elles sont soldées]** | **`dn5-3`** — ✅ soldé le 2026-09-04 |
| **le dépôt compile à froid** | 🔴 **NON MESURÉ** — ⛔ pas mesuré ici, et ⛔ pas affirmé | **`dn5-4`** |
| **le poids d'un clone** | 🔴 **NON MESURÉ** ici (bundle `main` ≈ 31 Mo au cadrage) **[ANNOTÉ LE 2026-09-05 (`dn5-5`) : ✅ **MESURÉ** désormais, par un `git clone` RÉEL dans un dossier neuf hors de l'arborescence, ⛔ pas par `du`. **Clone complet : 31,94 Mio reçus** (3 875 objets, 451 commits, les 2 branches publiées) · **`--depth 1` : 30,22 Mio** (637 objets). Les deux sont publiés dans `CONTRIBUTING.md` § *What cloning this costs*, chacun **avec à qui il sert** et **avec la commande qui le reproduit**. L'écart avec `du -sb .git` y est expliqué **objet par objet**. 🔴 **[CORRIGÉ LE 2026-09-05 À LA REVUE — la phrase d'origine est conservée juste après, et elle était FAUSSE]** : mesuré à `76b031a`, `du -sb .git` = **134 406 999 o**, écart = **100 701 660 o**, qui se répartit en **41,4 % de FORME DE STOCKAGE** (les mêmes 3 875 objets pèsent 75 146 810 o en local — dont 1 994 **libres**, zlib sans delta — contre 33 490 599 o dans le pack neuf du clone) et **58,0 % de CONTENU QU'AUCUN CLONE NE REÇOIT** (6 084 objets, dont **6 062 accessibles depuis AUCUNE ref** : du rebut d'historique). ⚠️ **Une fermeture « à l'octet » ne prouve RIEN** — `pack + libres + reste = .git` est une **identité de partition**, vraie quelles que soient les étiquettes ; ce qui est falsifiable, c'est l'**appartenance** d'un objet à un ensemble, et chaque ligne porte la commande qui la tranche. ⛔ **Phrase d'origine, ⛔ NON EFFACÉE** : *« l'écart est expliqué à l'octet — il n'a pas une seule cause : les objets libres n'en portent que 46,7 %, la moitié la plus grosse étant que le pack local couvre 4 branches locales qu'un clone ne demande jamais »*. **Les deux causes sont réfutées** : 1 994 des 2 077 objets libres **sont** accessibles depuis les refs publiées (leur contenu **est** transféré, empaqueté), et les branches locales ne portent que **22** objets, ⛔ pas ~4 000. ⚠️ L'ordre de grandeur « ≈ 31 Mo » du cadrage était **juste** ; ce qu'il ne disait pas, c'est **à qui chaque chiffre sert** et que **`--depth 1` ne fait gagner que 1,72 Mio, soit 5,4 % SUR LE FIL** — établi par un **TROISIÈME clone** (`--single-branch --branch main`, profondeur pleine) qui reçoit **exactement les mêmes 3 875 objets et 31,94 Mio** que le clone complet : la seconde branche publiée coûte **zéro**, donc tout l'écart est bien la **profondeur**. ⚠️ **Les deux premiers clones différaient sur DEUX variables** (profondeur ET périmètre) et ⛔ n'établissaient donc pas cette conclusion. ⚠️ Et la comparaison se fait **sur le fil contre sur le fil** (30,22 Mio ⇄ 31,94 Mio), ⛔ pas contre une taille apparente : celle de l'arbre est une **autre grandeur**, **46 254 844 o** à `76b031a`. Captures : `mesures/dn5-5/T1-clone-complet.txt`, `T2-clone-depth1.txt`, `T9-profondeur-vs-perimetre.txt`, `T8-decomposition-ecart-REFUTE-T3.txt`, `T10-volumetries-arbre-porteur.txt` *(⛔ `T3-ecart-du-vs-clone.txt` est conservée INTACTE mais sa décomposition est RÉFUTÉE — voir T8)*. ⛔ La ligne se DATE, elle ne se réécrit pas]** | **`dn5-5`** — ✅ soldé le 2026-09-05 |
...
> ⚠️ **ANNOTÉ LE 2026-09-04 par `dn5-3` — ⛔ LE TABLEAU CI-DESSOUS N'EST PAS RÉÉCRIT,
> il est DATÉ.** Il disait vrai le 2026-09-02. Depuis, **deux de ses quatre lignes sont
> tombées sans que ce soit cette story** : `verif_dossier_dn415.py` et
> `verif_ledger_dn416.py` ont été réparées par `dn4-40` (commit `f4848e4`) — elles
> dérivent leur racine de `__file__` et prennent `--cockpit`.
> 🔴 **CORRIGÉ À LA REVUE DU 2026-09-04 — ⛔ la ligne ci-dessus n'est pas effacée, elle
> nomme la mauvaise story.** Mesuré : à `f4848e4^` les deux fichiers portent **déjà** la
> racine `__file__`, `--cockpit` et le défaut dérivé de `HOME`. Ces trois-là arrivent le
> **2026-08-30** (`eb61b58` = `dn4-15`, `7f7f946` = `dn4-16`) ; le littéral du chemin
> personnel tombe à **`f5e9d69`, 2026-09-02 — `dn4-39`**. ⇒ **c'est `dn4-39` qui les a
> refermées**, ⛔ pas `dn4-40`. Les chiffres ne bougent pas ; le porteur, si. Et **une cinquième, jamais
> nommée ici**, a été trouvée le 2026-09-04 par un motif que personne n'avait cherché — le
> compte **Windows** de l'auteur : `tools/thermique_ventilos_dn48.py:310`.
> ⇒ **la classe 1 du 2026-09-04 compte TROIS outils, et ⛔ plus les mêmes** :
> `verif_dossier_d5_dn45.py`, `bench_lisseur_dn45.py`, `thermique_ventilos_dn48.py`.
> **Les trois sont soldées par `dn5-3`.**
> 🎯 **Et le constat de la ligne juste au-dessus est ce qui a fait la story** : la mesure
> prise sur la machine de l'auteur ne pouvant pas révéler un chemin absolu, `dn5-3` a
> construit un second étage de témoin — `/home/<auteur>` rendu **inatteignable** par
> `unshare -r -m` + `mount --bind`. Sans lui, `verif_dossier_d5_dn45.py` restait **VERTE**
> depuis un clone déplacé, en certifiant l'arbre de l'auteur. Relevé :
