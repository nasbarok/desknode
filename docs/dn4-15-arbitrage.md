...
| desknode | docs/journal-de-bord.md | 1 | busid-en-dur | 1 | > ⛔ pas une perte. **12 passages, 0 échec**, busid `3-5`, SHA au dossier §25.18. | HISTORIQUE | mention DATEE d'un busid, ⛔ pas une recette nue : elle se LIT, elle ne s'execute pas. Rendue visible par le motif elargi — ⚠️ *re-ciblé le 2026-09-13 (`dn8-4`) : la colonne fichier valait `README.md` ; la ligne est partie octet pour octet dans le journal* |
...
|---|---|---:|---|---:|---|---|---|
| desknode | docs/journal-de-bord.md | 1 | facade | 1 | côté de la tour PC** (décision owner du 2026-08-17 — le brief dit encore « monté sur la façade du | VRAI | la NOTICE de correction : le README dit « posé à côté », décision owner du 2026-08-17, et signale que le brief était périmé — ⚠️ *re-ciblé le 2026-09-13 (`dn8-4`) : la colonne fichier valait `README.md` ; la ligne est partie octet pour octet dans le journal* |
| desknode | docs/journal-de-bord.md | 1 | vl53l0x | 1 | \| **`tof`** (dn4-7) \| le **VL6180X** `0x29` instruit **pour de bon** — `etat` (registres, **lecture seule**), `sr03`,  | VRAI | le PIÈGE DE SOURCE : deux URL nommées « VL6180X » rendaient en HTTP 200 un vrai PDF ST… du VL53L0X — ⚠️ *re-ciblé le 2026-09-13 (`dn8-4`) : la colonne fichier valait `README.md` ; la ligne est partie octet pour octet dans le journal* |
| desknode | docs/journal-de-bord.md | 1 | vl53l0x | 1 | \| **`i2c`** (dn2-1/**dn4-2**) \| le **bus vu de ses adresses** : `i2c` scanne 0x08..0x77, `i2c lire <addr> <reg> [n]` l | VRAI | le TÉMOIN NÉGATIF de l'identification (`i2c lire 29 C0` ne doit PAS rendre `EE` de façon reproductible) — ⚠️ *re-ciblé le 2026-09-13 (`dn8-4`) : la colonne fichier valait `README.md` ; la ligne est partie octet pour octet dans le journal* |
| desknode | docs/journal-de-bord.md | 1 | facade | 1 | ⛔ **ET UN VENTILATEUR NE SERA JAMAIS PUBLIABLE** : le **200 mm de façade** n'a **pas de fil | VRAI | le VENTILATEUR 200 mm DE FAÇADE de la tour — fait MESURÉ (pas de fil tachymétrique, `0 RPM` dans le BIOS aussi). ⛔ NE PAS TOUCHER — ⚠️ *re-ciblé le 2026-09-13 (`dn8-4`) : la colonne fichier valait `README.md` ; la ligne est partie octet pour octet dans le journal* |
| desknode | docs/journal-de-bord.md | 1 | vl53l0x | 1 | \| 🔴 **TOF050C-VL6180X** (⛔ **PAS** un VL53L0X) \| proximité / présence \| **`0x29` MESURÉ** ✅ **QUALIFIÉ PAR LECTURE**  | VRAI | la CORRECTION elle-même : « TOF050C-VL6180X (⛔ PAS un VL53L0X) », qualifié par lecture — ⚠️ *re-ciblé le 2026-09-13 (`dn8-4`) : la colonne fichier valait `README.md` ; la ligne est partie octet pour octet dans le journal* |
...
| desknode | docs/bom.md | 1 | vl53l0x | 1 | \| Distance sensor \| **TOF050C-VL6180X** (⛔ **NOT** a VL53L0X) \| **3.21 €** (1 listing naming it explicitly) \| 2026-09-06 | VRAI | la MISE EN GARDE D'ACHAT de la page BOM : la reference a commander est un TOF050C-VL6180X, et le nom voisin n'est ecrit QUE pour etre ecarte. Occurrence NEE avec `docs/bom.md` (dn6-1, 2026-09-06) |
