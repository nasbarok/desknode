...
- la **console est interactive** : taper `aide` dans le moniteur liste les commandes. Jeu complet :
  ...
  `touch`, `nav`, `recal`, `bl`, `disp`, `dma`, `i2c` (`lire` · **`lire16`** · **`brut`** · **`ecrire`** · **`rafale`**), `capteurs`, **`env`**, **`tof`** (`etat` · `sr03` · `balayage` · `als` · `range`), **`w2`**, `pc`, `wifi`, `widget`, **`hist`**, `rtc`, `reboot`, `aide`. `cfg reset` rend
