...
- la **console est interactive** : taper `aide` dans le moniteur liste les commandes. Jeu complet :
  ...
  `touch`, `nav`, `recal`, `bl`, `disp`, `dma`, `i2c` (`lire` · **`lire16`** · **`brut`** · **`ecrire`** · **`rafale`**), `capteurs`, **`env`**, `pc`, `wifi`, `widget`, `rtc`, `reboot`, `aide`. `cfg reset` rend
  ...
  | `bl <0..100>` · `bl ramp <pct> [ms]` · `bl freq <hz>` · **`bl auto on\|off`** · **`bl auto bornes <bas> <haut>`** · **`bl auto pas <n>`** | rétroéclairage **gradable** et, depuis **dn4-3**, **asservi au BH1750**. ⚠️ `bl 0` = dalle NOIRE ; `disp off` = dalle GRISE éclairée. 🔴 **La loi est ÉCRITE AVANT la mesure et ses quatre bornes sont ancrées sur des mesures de ce dépôt** : **3 %** à ≤ **20 lx** (le plancher LISIBLE, *constat owner* d'AC7 de dn1-3 — « le Living PCB et le label s'y distinguent encore, TOUT JUSTE »), **100 %** à ≥ **400 lx** (l'éclairage de bureau MESURÉ sur cette carte le 2026-08-20 : brut 494 ⇒ 411 lx), interpolation linéaire entre les deux. **Bande morte 3 points** (= 31 crans LEDC sur 1 023 : en dessous, l'œil ne peut pas voir la correction), **pas maximal 20 points par cycle de 5 s** ⇒ course complète en **5 cycles = 25 s**. ⛔ **`dn_display_backlight_ramp()` n'est JAMAIS appelée par l'asservissement** : elle est BLOQUANTE (`vTaskDelay`) et son docblock la déclare « geste d'OPÉRATEUR, pas un effet de fond ». 🔴 **`bl <n>`, `bl on|off` et `bl ramp` DÉSARMENT l'auto ET LE DISENT** — `dn_display_backlight_pct()` n'a **aucun verrou** et `s_backlight_pct` est un `int` nu ; sans ce désarmement, un `bl 50` tapé en séance serait **écrasé au cycle suivant SANS UN MOT**, et le constat owner mesurerait **la boucle en croyant mesurer la commande**. ⚠️ **Capteur muet ⇒ le duty NE BOUGE PAS** (journalisé une fois) : un capteur silencieux ne doit ni éteindre l'écran ni le mettre à fond. 🔴 **DÉSARMÉ PAR DÉFAUT** — c'est le repli pré-autorisé d'AC5 qui devient l'état de départ, la discipline de boot reste intouchée, et l'A/B se joue **dans UN SEUL firmware**. Bornes et pas **réglables à chaud**, parce que l'arbitrage se tranche **sur la dalle**, pas au papier |
